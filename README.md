...
| `edit_bonds` | Add and/or remove bonds by atom index pairs (`add`, `remove`, `bond_type`), as the Bond Editor plugin does: 3D coordinates kept, one undo step; an edit that breaks valence rules (an atom above its usual valence) is kept unsanitized and reported |
